...
full_data <- read_csv(file.path(fp_pa,'RIWH_NOV_2019_PRES_CLIMATOLOGY.csv'))
...
dir.create(file.path(DIR, 'paper_2050_models_70_data_split'), showWarnings = FALSE)
#Create reference variable
fp_model <- file.path(DIR, 'paper_2050_models_70_data_split')
...
species_name <- 'RIWH.2050.Paper.70.Data.Split'
...
for (month in 1:12) {
  ...
  trainingCovars <- as.data.frame(trainingData[,c('sst', 'sst_btm', 'bat', 'cal')])
  ...
  #Assign layer names 
  names(proj_covars) <- c('sst', 'sst_btm', 'bat', 'cal')
  ...
  #Ji et al. 2011 life history model
  cal_2050 <- raster(file.path(fp_cal_2050, paste('calanus_', month, '.grd', sep = ''))) %>%
    raster::resample(sst)
  #Zero out values at depths less than 80m
  cal_2050[bat >= -80] <- 0
  #Change NA values to 0
  cal_2050[is.na(cal_2050)] <- 0
  ...
  names(proj_covars_2050_R45) <- c('sst', 'sst_btm', 'bat', 'cal')
  #Assign layer names for R85
  names(proj_covars_2050_R85) <- c('sst', 'sst_btm', 'bat', 'cal')
